...
async function convertToAvif(file, options = {}) {
  ...
  try {
    ...
    if (img.width < config.minWidth || img.height < config.minHeight) {
      URL.revokeObjectURL(img.src)
      throw new Error(
        `Image dimensions (${img.width}x${img.height}) are too small. Minimum required is ${config.minWidth}x${config.minHeight}`,
      )
    }
  ...
  async uploadPicture(file, pictureType = 'avatar', options = {}) {
    try {
      ...
      const {
        ...
        isAvif,
        dimensions,
      } = await convertToAvif(file, options)
...
export function usePicture() {
  ...
  async function uploadPicture(file, pictureType = 'profile', options = {}) {
    ...
    try {
      ...
      // Update picture URL
      pictureUrl.value = await PictureService.retrievePicture(
        null,
        pictureType,
        'url',
      )
